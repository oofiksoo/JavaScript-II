...
const firstNamesAllCaps = runners.map(function(value) {
...
let runnersLargeSizeShirt = runners.filter(function(shirts) {
...
// Problem 1
//count of people dontation under 10 (.filter)
let donationunder10 = runners.filter(function(donationtots) {
...
const withSponsor = [];
runners.forEach(function(n1) {
    withSponsor.push(n1.first_name + " " + n1.last_name + " " + n1.company_name);
});
console.log(withSponsor);
// Problem 3
//list of emails for bulk thanks message .map()
const emailList = runners.map(function(value) {
    return value.email;
});
